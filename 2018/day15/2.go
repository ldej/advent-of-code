...
func main() {
	...
	for !found {
		...
		elvenBoost += 1
		found = do(grid, elvenBoost)
	}
	fmt.Println("Wrong:\nElven attack: 14\n48 * 1214 = 58272\n")
}

func do(grid []string, elvenBoost int) bool {
	var c characters

	c, grid = findCharacters(grid, elvenBoost)
	...
	for !somebodyWon {
		...
		for idx := 0; idx < len(c); idx++ {
			...
			c, updateIndex, killed = attack(char, c)
...
func attack(char character, c characters) (characters, bool, character) {
	...
	if enemies, foundAny := nextTo(char, enemies); foundAny {
		...
		if lowestHP.HitPoints <= 0 {
			return removeCharacter(c, lowestHP), lowestHP.Point.ComesBefore(char.Point), lowestHP
		}
		return replaceCharacter(c, lowestHP), false, character{}
	}
	return c, false, character{}
}
...
func findCharacters(grid []string, elvenBoost int) (characters, []string) {
	c := characters{}
